...
def index():
    """Главная страница"""
    print(f"🌐 Запрос на главную страницу от {request.remote_addr}")
    try:
        # Проверяем что HTML_TEMPLATE существует
        if not HTML_TEMPLATE:
            return "<h1>Ошибка: HTML_TEMPLATE не определен</h1>"
        
        print(f"📄 Генерируем HTML страницу ({len(HTML_TEMPLATE)} символов)")
        ...
        error_msg = f"<h1>Ошибка загрузки дашборда</h1><p>{str(e)}</p><p>Проверьте консоль на предмет ошибок.</p>"
...
def test_page():
    ...
    <body>
        ...
        <p>Если вы видите эту страницу, Flask работает корректно.</p>
        <p><a href="/">Вернуться к дашборду</a></p>
        <script>
...
if __name__ == '__main__':
    ...
    port = 5005  # Используем другой порт
    print(f"🌐 Дашборд доступен по адресу: http://localhost:{port}")
    print(f"🌐 Альтернативный адрес: http://127.0.0.1:{port}")
    print("⏹️  Для остановки нажмите Ctrl+C")
    print("💡 Если видите код HTML в браузере:")
    print("   1. Очистите кеш браузера (Ctrl+F5 или Cmd+Shift+R)")
    print("   2. Попробуйте режим инкогнито")
    print("   3. Попробуйте другой браузер")
    print("   4. Проверьте что заходите именно на указанный порт")
