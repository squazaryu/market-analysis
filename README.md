...
pip install flask pandas requests beautifulsoup4 numpy plotly

# Альтернативно, если есть requirements.txt
...
1. ✅ Загрузит исторические данные с MOEX API
2. ✅ Получит актуальные данные о СЧА и ценах паев с investfunds.ru  
3. ✅ Рассчитает комиссии и эффективность фондов
4. ✅ Создаст локальный кеш для быстрого доступа

**Время первого запуска:** 2-3 минуты (загрузка данных всех 96 фондов)

### Доступ к дашборду
- **Главная страница:** http://localhost:5004
- **API статуса:** http://localhost:5004/api/status  
- **API обновления:** http://localhost:5004/api/update-data
- **Анализ комиссий:** http://localhost:5004/api/fee-analysis

### Автоматическое обновление
Система автоматически обновляет данные:
- **Кеш investfunds.ru:** обновляется ежедневно
- **Исторические данные MOEX:** обновляются при запросе
- **Принудительное обновление:** через `/api/update-data`
